                ...
                with gr.Row():
                    with gr.Column():
                        ...
                        opt_cut_by_sent = gr.Checkbox(
                            label="按句切分    在按段落切分的基础上再按句子切分文本",value=True
                        )

                        slicer = gr.Button("切分生成推理(速度快)", variant="primary")

                        stream_button = gr.Button("流式推理,即一边推理一边播放", variant="primary")

                text_output = gr.Textbox(label="状态信息")
                audio_output = gr.Audio(label="输出音频",streaming=True,
                autoplay=True,
                interactive=False,
                show_label=True,
